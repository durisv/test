...
- Create internal windows machine (with WSL) that will be used to access jump-server
- Install **VS Code** with **Remote - SSH** extension on your windows machine
...
>- When using WSL don't clone the Cloud Azure resources repository to folder mounted from Windows. Ideally use your home (~) folder.
>- **We recommend to use VS Code to connect to WSL or your linux machine. You will have available all VS Code tools that way.**

1. Generate new SSH key pair that will be used as a deploy key

    ```bash
    mkdir ~/.ssh/cloud-azure-resources
    ssh-keygen -f ~/.ssh/cloud-azure-resources/id_rsa -N ""
    cat ~/.ssh/cloud-azure-resources/id_rsa.pub
    ```

2. Upload the cloud-azure-resources key
3. Open this URL: <https://github.com/2Ring/cloud-azure-resources>
4. Settings -> Security -> Deploy keys
5. Create new Deploy key
    - Title: subscription name (e.g. dw-us-east)
    - Key: Use content of this file: ~/.ssh/cloud-azure-resources/id_rsa.pub
    - Allow write access: true

6. Clone the cloud-azure-resources repository

    ```bash
    mkdir -p ~/cloud-azure-resources/{subscription name}

    git config --global --add advice.detachedHead false
    git config --global user.name "Your Name"
    git config --global user.email you@example.com

    GIT_SSH_COMMAND='ssh -i ~/.ssh/cloud-azure-resources/id_rsa -o IdentitiesOnly=yes' git clone -q -b spring22 git@github.com:2Ring/cloud-azure-resources.git ~/cloud-azure-resources/{subscription name}
    ```

- Create new branch

   ```bash
   cd ~/cloud-azure-resources/{subscription name}
   git checkout -b {subscription name}
   ```

- Push local branches to origin (create branches on server)
...
2. Create the DW resource group. Use location values from this command: az account list-locations --output=table.

    ```bash
    az group create -l {location} -n dw
    ```

3. Create the Azure Container Registry. Parameter 'name' must conform to the following pattern: '^[a-zA-Z0-9]*$'.

    ```bash
    az acr create --name {name} --resource-group dw --sku basic
    ```

4. After the Azure Container Registry is created, push images to the registry (kube/images/push_images_to_production_repository.py).
...
3. Enter the correct AZ region to settings.json
...
4. ***Optional***: Edit the default-settings.json configuration file.
...
5. Create ssh key for jump-server
...
6. Create DNS entry for jump-server pointing to jump-server public IP (can be find out on jump-server overview in azure portal)

    ```bash
    {region}-jump.2ring.cloud
    ```

7. Enable the Azure just-in-time access.
8. In the Azure portal, open the Microsoft Defender for Cloud blade and click the Upgrade button.
9. Open the jump-server blade, choose Configuration and then click the Enable Just-in-time button.
10. In the jump-server blade, click the Connect button, choose SSH, from Source IP select My IP and request the just-in-time access (default
3 hours).
